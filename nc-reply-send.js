...
The domain name associated with this Namecheap account is: mexriddin.online

My account username is: oybek2011.

Please temporarily disable the additional verification step so I can log in and update my account email address to oybekabdurasulov431@gmail.com.

If you require payment details for further verification, please let me know exactly what information you need.
...
(async () => {
  ...
  const clicked = await page.evaluate(() => {
    ...
    for (const r of rows) {
      if (/account access/i.test(r.innerText)) { r.click(); return r.innerText.replace(/\s+/g, ' ').slice(0, 120); }
    }
    return null;
  });
